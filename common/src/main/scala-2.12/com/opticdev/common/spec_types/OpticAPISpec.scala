...
import com.opticdev.common.utils.StringBuilderImplicits._
...
object OpticAPISpec {
  ...
  def specToSwagger(opticAPISpec: OpticAPISpec): JsObject = {
    val securitySchemes = authToSwagger(opticAPISpec.authenticationSchemes)
  ...
  def parameterToSwagger(parameter: Parameter) =
    SwaggerParam(parameter.name, parameter.in, parameter.required, parameter.schema.getOrElse(JsObject.empty))

  def bodyToSwagger(requestBody: RequestBody) =
    SwaggerRequestBody(Map(requestBody.`content-type`.getOrElse("application/json") -> requestBody.schema.getOrElse(JsObject.empty)))
