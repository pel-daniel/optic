...
class ToSwaggerSpec extends FunSpec {
  ...
  val endpoints = Vector(
    Endpoint("get", "/url/:hello/", Vector(
      Parameter("query", "filter", true, None),
      Parameter("query", "filterA", true, None),
      Parameter("query", "filterB", true, None),
      Parameter("query", "filterC", true, None),
    ),
      body = Some(RequestBody(None, Some(Json.parse(
        """
          |{
          |					"type": "object",
          |					"properties": {
    ...
    Endpoint("post", "/url/:hello/", Vector(Parameter("header", "X-AUTH", true, None)), responses = Vector(
